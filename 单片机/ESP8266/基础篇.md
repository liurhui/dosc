...
```plaintext
	...
	可以设置外部中断触发模式去执行、或者配置定时器 周期性去执行。
```
...
wififi_softap_set_confifig(设置配置，并保存到 Flash)
...
wififi_softap_set_confifig_current
```c
/*
	请在使能的情况下，调⽤本接⼝。
...
```
十、wififi_softap_get_station_num（获取 ESP8266 SoftAP 下连接的 Station 个数）
```c
