#### 一、了解
	目的作用：类似js的npm或yarn包管理工具,c++的 makefile 。
	目录结构：有一定的规范要求。在project根目录下。
	指令：
```xml
	mvn -v
	mvn compile  //将java源文件编译成class文件
	mvn package  //项目打成jar包
	mvn  clean //删除target文件夹
    mvn install //将当前项目放到Maven的本地仓库中。供其他项目使用
...
```
	4、依赖冲突执行逻辑: 
		短路优先：直接找最短的依赖文件。
		声明优先：引用路径长度相同，根据pom.xml 先声明为主。
	5、聚合：多项目同时运行
		只要在pom.xml中配置modules 。
```xml
<modules>
